    ...
    <AIChatProvider value={{ openChat }}>
      <div className="h-screen flex overflow-hidden bg-background">
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-background">
          <header className="h-16 md:h-20 bg-transparent flex items-center justify-between px-4 md:px-8 gap-3 md:gap-4 shrink-0 z-10 pt-2 md:pt-4">
            <div className="flex items-center gap-3 md:gap-4">
              <button className="lg:hidden p-2 -ml-2 rounded-lg hover:bg-muted" onClick={() => setSidebarOpen(true)}>
                <Menu className="h-5 w-5" />
              </button>
              <h2 className="text-xl md:text-2xl font-bold tracking-tight text-foreground hidden sm:block">
            ...
            <div className="flex flex-1 items-center justify-end gap-2 md:gap-5">
              ...
              <button className="relative p-2 rounded-xl text-muted-foreground hover:bg-muted hover:text-foreground transition-colors hidden sm:block">
          ...
          <main className="flex-1 overflow-y-auto p-3 sm:p-4 md:p-6 lg:p-8 pt-2 md:pt-4">
            {children}
          </main>
        </div>
